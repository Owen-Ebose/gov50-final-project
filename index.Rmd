...
census <- census |>
  mutate(employment_type = case_when(
    employment_class == "Federal gov't" | employment_class == "State gov't" | employment_class == "Local gov't" | employment_class == "Non profit" ~ "Not company",
...
diff_in_type_rates <- employment_type_turnout |>
  pivot_wider(names_from = employment_type,
              values_from = turnout) |>
  mutate(difference = `Not company` - Company) |>
  select(difference)
diff_in_type_rates
```
Those not working at a company turned out at a rate 16.01% higher than those working at a company in 2022. Workers for the federal government, state government, local government and nonprofits all turned out at rates higher than 70%, meanwhile workers at forprofit companies turned out at a rate of 57%. This suggests that there is something unique about working at a forprofit company that inhibits one's ability or willingness to participate in elections. Every other kind of worker participates much more. 

I confirm the results using confidence interval/ hypothesis test

It could be income that is driving this effect, rather than the factors I identified in my hypothesis. 

I first calculate the average household income between the employment types and classes. It turns out that those at companies make the least income. 

Does that matter? I find out by calculating turnout rates within income groups to estimate the effect of income on turnout. It turns out higher income groups turnout more. 

I confirm this association via a regression analysis. 

How can I gauge effect of employment type in light of this confounder? Calculate turnout rates of employment types within each income group. Turns out that the effect of employment type persists within each group. 
...
```{r}
#comparing average household income across employment types
...
```{r}
#calculating turnout rates of employment classes within income groups

class__income_turnout <- census |>
  group_by(broad_income_categories, employment_class) |>
  summarize(turnout = mean(voted_num) * 100)

custom_levels <- c("150,000 or more","100,000 to 149,999","60,000 to 99,999", "35,000 to 59,999", "15,000 to 34,999", "Below 15,000")
class__income_turnout$broad_income_categories <- factor(class__income_turnout$broad_income_categories, levels = custom_levels)

knitr::kable(class__income_turnout, digits = 2)

ggplot(class__income_turnout, aes(x = fct_reorder (employment_class, turnout), y = turnout)) +
  geom_col(aes(fill = broad_income_categories), position = "dodge") +
  facet_wrap(~broad_income_categories, nrow=2, scales = "fixed") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Employment class", y = "Turnout (%) in 2022 national elections")
